...
{
public: 
    PCDSubscriber() : Node("pcd_subsriber") {
        ...
        this->declare_parameter<std::string>("field_to_save", "intensity"); // New parameter
        ...
        this->get_parameter("field_to_save", field_to_save_); // Get the new parameter
...
private:
    void appendPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        if (!has_field) {
            RCLCPP_WARN(this->get_logger(), "PointCloud2 message does not contain the '%s' field. Data discarded!", field_to_save_.c_str());
            return;
        }
    ...
    string field_to_save_; // New member variable
};
